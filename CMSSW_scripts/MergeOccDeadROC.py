...
def main():
    ...
    dirFED="DQMData/Run " + runNum + "/PixelPhase1/Run summary/FED/"
    ...
    for lyr in range(1,5):
        ...
        histDeadList.append(rootf.FindObjectAny(hdeadB+str(lyr)))
            ...
            if match != None and "per_SignedModuleCoord_per_SignedLadderCoord" in hist1.GetName():
                ...
                for biny in range(1,hist2.GetNbinsY()+1):
                    if len(binTBM)!=0:
                        ...
                        if hist2.GetBinContent(binTBM[0],biny-1) >= 0.97*hist2.GetMaximum():
                            draw_box(boxList,x1,x2,y1,y2,0.2,1,0,1,lineWd)
                        else:
                            draw_box(boxList,x1,x2,y1,y2,0.2,633,0,1,lineWd)
                    binTBM=[]
                        ...
                        if hist2.GetBinContent(binx,biny)!=0:
                            if len(binTBM)==0:
                                ...
                                if len(binTBM)==tbmRoc: 
                                    ...
                                    if hist2.GetBinContent(binTBM[0],biny) >= 0.97*hist2.GetMaximum():
                                        draw_box(boxList,x1,x2,y1,y2,0.2,1,0,1,lineWd)
                                    else:
                                        draw_box(boxList,x1,x2,y1,y2,0.2,633,0,1,lineWd)
                                    binTBM=[]
                            ...
                            if len(binTBM)!=0:
                                ...
                                if hist2.GetBinContent(binTBM[0],biny) >= 0.97*hist2.GetMaximum():
                                    ...
                                    draw_box(boxList,x1,x2,y1,y2,0.2,633,0,1,lineWd)
                #c.SaveAs('MergedOccupancyDeadROC_BPix_Layer{0}_TBM.pdf'.format(lyr))
                ...
                c.SaveAs('MergedOccupancyDeadROC_BPix_Layer{0}_TBM.png'.format(lyr))
        else :
                        ...
                        if hist2.GetBinContent(binx,biny)!=0:
                            ...
                            if len(binTBM)!=0:
                                ...
                                if hist2.GetBinContent(binTBM[0],biny) >= 0.97*hist2.GetMaximum():
                                    ...
                                    draw_box(boxList,x1,x2,y1,y2,0.2,633,0,1,lineWd)
                #c.SaveAs('MergedOccupancyDeadROC_FPix_Ring{0}_TBM.pdf'.format(ring))
                #os.system('gs -dBATCH -dNOPAUSE -sDEVICE=png16m -dUseCropBox -sOutputFile=MergedOccupancyDeadROC_FPix_Ring{0}_TBM.png -r144 -q MergedOccupancyDeadROC_FPix_Ring{0}_TBM.pdf'.format(ring))
                #os.system('rm -f MergedOccupancyDeadROC_FPix_Ring{0}_TBM.pdf'.format(ring))
                c.SaveAs('MergedOccupancyDeadROC_FPix_Ring{0}_TBM.png'.format(ring))
