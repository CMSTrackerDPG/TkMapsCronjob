...
def setRunDirectory(runNumber):
    ...
    dirDict = { 
        ...
        394050:['Data2025', 'OORun2025'],\
        393730:['Data2025', 'pORun2025'],\
        390740:['Data2025', 'Run2025'],\
	389350:['Data2025', 'Commissioning2025'],\
        387853:['Data2024', 'HIRun2024'],\
	378142:['Data2024', 'Run2024'],\
	376685:['Data2024', 'Commissioning2024'],\
        373881:['Data2023', 'HIRun2023'],\
        365740:['Data2023', 'Run2023'],\
        363380:['Data2023', 'Commissioning2023'],\
        362433:['Data2022', 'Run2022'],\
        362192:['Data2022', 'HIRun2022'],\
        352318:['Data2022', 'Run2022'],\
        347687:['Data2022', 'Commissioning2022'],\
        343006:['Data2021', 'Commissioning2021'],\
        335443:['Data2020', 'Commissioning2020'],\
        328588:['Data2019', 'Commissioning2019'],\
...
def downloadOfflineDQMhisto(run, Run_type,rereco):
    ...
    url1 = 'https://cmsweb.cern.ch/dqm/offline/data/browse/ROOT/OfflineData/'+DataOfflineDir+'/'+Run_type+'/000'+str(nnn)+'xx/'
    subprocess.run(f"curl -k --cert /tmp/x509up_u133079 --key /tmp/x509up_u133079 -X GET {url1} > {index_file}", shell=True, check=True)
    ...
    for _ in range(10):
        ...
        raise RuntimeError(f"Failed to download {index_file}")
    ...
    if File_Name:
        print(f"Downloading DQM file: {File_Name}")
        subprocess.run(
            f"curl -k --cert /tmp/x509up_u133079 --key /tmp/x509up_u133079 -X GET https://cmsweb.cern.ch/dqm/offline/data/browse/ROOT/OfflineData/{DataOfflineDir}/{Run_type}/000{nnn}xx/{File_Name} > /tmp/{File_Name}",
            shell=True,
            check=True,
        )
        return File_Name
    else:
        print("No valid DQM file found.")
        return None
...
def downloadOnlineDQMhisto(run, Run_type):
    ...
    DataOfflineDir=runDirval[1]
    nnn=int(run/100)
    nnnOnline = int(run/10000)
    deadRocMap = False
    File_Name_online=None
    index_file = "index_online.html"
    ##################online file########    
    ...
    if any(str(run) in x for x in index_online):
        ...
        subprocess.run(f"curl -k --cert /tmp/x509up_u133079 --key /tmp/x509up_u133079 -X GET {url2} > index_online_backup.html", shell=True, check=True)

        # Wait for the backup file
        for _ in range(10):
            if os.path.exists("index_online_backup.html") and os.path.getsize("index_online_backup.html") > 0:
                break
            time.sleep(1)
    ...
    if File_Name_online:
        print(f"Downloading DQM file: {File_Name_online}")
        subprocess.run(
            f"curl -k --cert /tmp/x509up_u133079 --key /tmp/x509up_u133079 -X GET https://cmsweb.cern.ch/dqm/online/data/browse/Original/000{nnnOnline}xxxx/000{nnn}xx/{File_Name_online} > /tmp/{File_Name_online}",
            shell=True,
            check=True,
        )
        ...
        print("No valid DQM file found.")
        return deadRocMap, None

def downloadOnlineHLTDQMhisto(run, Run_type):
    runDirval=setRunDirectory(run)
    DataLocalDir=runDirval[0]
    DataOfflineDir=runDirval[1]
    nnn=int(run/100)
    ...
    with codecs.open(index_file, "r") as f_online:
        index_online = f_online.readlines()

    if any(str(run) in x for x in index_online):
        for x in index_online:
            if (str(run) in x) and ("_HLT_" in x):
                File_Name_online=str(str(x).split(".root'>")[1].split("</a></td><td>")[0])
    else:
        print("Can't find any file in offline server, trying the online server")
        url2 = 'https://cmsweb.cern.ch/dqm/offline/data/browse/ROOT/OnlineData/original/000'+str(nnnOnline)+'xxxx/000'+str(nnn)+'xx/'
        ...
        with codecs.open("index_online_backup.html", "r") as f_online_backup:
            index_online_backup = f_online_backup.readlines()
            
        if any(str(run) in y for y in index_online_backup):
            for y in index_online:
                if (str(run) in y) and ("_HLT_" in y):
                    File_Name_online=str(str(y).split(".root'>")[1].split("</a></td><td>")[0])
        else:
            print('No HLT Online DQM file available. Skip HLT Strip TkMap')
...
log_path = "/data/users/event_display/dpgtkdqm/cronjobs/cronlogs/"
log_file = log_path+"TkMaps_cron.log"
timestamp = datetime.now().strftime("%m/%d/%Y %H:%M:%S")
log_message = f"{timestamp} - TkMaps starting\n"

with open(log_file, "w") as f: 
    f.write(log_message)
    
###########Check if user enter the right run type######################
if Run_type == 'Cosmics' or Run_type == 'StreamExpress' or Run_type == 'StreamExpressCosmics' or Run_type == 'ZeroBias'  or Run_type == 'StreamHIExpress' or Run_type == 'StreamHIExpressRawPrime' or Run_type == 'HIMinimumBias1' or re.match('ZeroBias([0-9]+?)',Run_type) or re.match('HIMinimumBias([0-9]+?)',Run_type) or re.match('HIPhysicsRawPrime([0-9]+?)',Run_type):
    print(Run_type)
...
#################Downloading DQM file############################
    ...
    deadRocMap, File_Name_online = downloadOnlineDQMhisto(Run_Number[i], Run_type)
...
################Check if run is complete##############
    ...
    if Check_output == 0:
        ...
        print('*******You will need to re-run once processing is complete*************')
        time.sleep(2.5)
    ...
    if Run_type=="StreamExpress" or Run_type=="StreamHIExpress":
        if File_Name_PCL=='Temp':
            ...
            print('*******You will need to re-run the script later for PCL plots**********')
...
#######Getting GT##############
    ...
    globalTag = getGTfromDQMFile(filepath+File_Name, str(Run_Number[i]), 'Globaltag')
    print(globalTag)
    if "DEFAULT" in globalTag: globalTag="132X_dataRun3_Prompt_v2"
    ####################################################
...
## create merged list of BadComponent from (PCL, RunInfo and FED Errors) ignore for now
    ...
    print('Creating directory:{}/{}/{}/{}/{}/{}'.format(OUTPUTDIR,DataLocalDir,dest,str(nnnOut), str(Run_Number[i]), Run_type))
    os.system("mkdir -p {}/{}/{}/{}/{}/{} 2>/dev/null".format(OUTPUTDIR,DataLocalDir,dest, str(nnnOut), str(Run_Number[i]), Run_type))
...
##############counting dead pixel#######################
    ...
    if rereco:
        os.system("mkdir -p {}/{}/{}/{}/{}/ReReco 2>/dev/null".format(OUTPUTDIR,DataLocalDir,dest, str(nnnOut), str(Run_Number[i])))
    else:
        os.system("mkdir -p {}/{}/{}/{}/{}/{} 2>/dev/null".format(OUTPUTDIR,DataLocalDir,dest, str(nnnOut), str(Run_Number[i]), Run_type))
...
###################copy ouput files###################
    ...
    for file_name in strip_files:
        ...
        if (os.path.isfile(full_stripfile_name)):
            if rereco:
                shutil.copy(full_stripfile_name, "{}/{}/{}/{}/{}/ReReco".format(OUTPUTDIR, DataLocalDir, dest, str(nnnOut), str(Run_Number[i])))
            else:
                shutil.copy(full_stripfile_name, "{}/{}/{}/{}/{}/{}".format(OUTPUTDIR, DataLocalDir, dest, str(nnnOut), str(Run_Number[i]), Run_type))
...
###################copy ouput files##########
    ...
    for file_name in pixel_files:
        ...
        if (os.path.isfile(full_pixelfile_name)):
            if rereco:
                shutil.copy(full_pixelfile_name, "{}/{}/{}/{}/{}/ReReco".format(OUTPUTDIR, DataLocalDir, dest, str(nnnOut), str(Run_Number[i])))
            else:
                shutil.copy(full_pixelfile_name, "{}/{}/{}/{}/{}/{}".format(OUTPUTDIR, DataLocalDir, dest, str(nnnOut), str(Run_Number[i]), Run_type))
    ...
    print("Producing pixel phase1 tree for Offline TkCommissioner from file:" + filepath+'/'+File_Name + ' ' + pixelTreeFileName)
    if not os.path.exists(filepath+'/'+File_Name + ' ' + pixelTreeFileName):
        print(filepath+'/'+File_Name + ' ' + pixelTreeFileName + ' does not exist')
    
    os.system('${CMSSW_BASE}/src/DQM/SiStripMonitorClient/scripts/PhaseITreeProducer.py ' + filepath+'/'+File_Name + ' ' + pixelTreeFileName)
        
    print("Dest file {}/{}/{}/{}/{}/{}/{}".format(OUTPUTDIR, DataLocalDir, dest, str(nnnOut), str(Run_Number[i]), Run_type, pixelTreeFileName))
    shutil.copy(pixelTreeFileName, "{}/{}/{}/{}/{}/{}/{}".format(OUTPUTDIR, DataLocalDir, dest, str(nnnOut), str(Run_Number[i]), Run_type, pixelTreeFileName))
    ...
    if Run_type in ["StreamExpress", "StreamExpressCosmics"] :
        print("Making maps for Strips@HLT")
        File_Name_hlt = downloadOnlineHLTDQMhisto(Run_Number[i], Run_type)
        print('Using HLT DQM file: '+File_Name_hlt)
        
        globalTag = getGTfromDQMFile(filepath+File_Name_hlt, str(Run_Number[i]), 'Globaltag', isHLT=True)
        print(globalTag)
            
        os.system('cmsRun ${CMSSW_BASE}/src/DQM/SiStripMonitorClient/test/mergeBadChannel_TemplateForHLT_cfg.py globalTag='+globalTag+' runNumber='+str(Run_Number[i])+' dqmFile='+filepath+'/'+File_Name_hlt)
        outFolder="{}/{}/{}/{}/{}/{}/".format(OUTPUTDIR,DataLocalDir,dest, str(nnnOut), str(Run_Number[i]), Run_type)
        os.system('rm -f *root')
        shutil.move('MergedBadComponents.log',outFolder+'MergedBadComponents_run'+str(Run_Number[i])+'_HLT.txt')
        shutil.move('MergedBadComponentsTkMap.png',outFolder+'MergedBadComponentsTkMap_HLT.png')
        shutil.move('MergedBadComponentsTkMap.xml',outFolder+'MergedBadComponentsTkMap_HLT.xml')
        shutil.move('MergedBadComponentsTkMap_Run_000000.xml',outFolder+'MergedBadComponentsTkMap_'+str(Run_Number[i])+'_HLT.xml')
        shutil.move('MergedBadComponentsTkMap_Run_000000.png',outFolder+'MergedBadComponentsTkMap_'+str(Run_Number[i])+'_HLT.png')
