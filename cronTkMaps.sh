...
python3 listProducer.py -y 2026 --min 400600
...
while read -r number; do
  source /data/users/event_display/dpgtkdqm/cronjobs/TkMapsCronjob/TkMapsCronjob/tkmapsFromCronjob.sh "$datasetExpress $number"
done < "$Express_file"
...
while read -r number; do
  source /data/users/event_display/dpgtkdqm/cronjobs/TkMapsCronjob/TkMapsCronjob/tkmapsFromCronjob.sh "$datasetPrompt $number"
done < "$Prompt_file"
...
while read -r number; do
  source /data/users/event_display/dpgtkdqm/cronjobs/TkMapsCronjob/TkMapsCronjob/tkmapsFromCronjob.sh "$datasetExpressCosmics $number"
done < "$Express_file"
...
while read -r number; do
  source /data/users/event_display/dpgtkdqm/cronjobs/TkMapsCronjob/TkMapsCronjob/tkmapsFromCronjob.sh "$datasetPromptCosmics $number"
done < "$Prompt_file"
